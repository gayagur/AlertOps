...
async def process_alert(db: AsyncSession, alert_data: dict):
    ...
    if not matched_subs:
        logger.info(f"No matching subscriptions for alert {alert.id}")
        return

    # Notify each matched subscriber
    total_sent = 0
    for sub in matched_subs:
        logs = await notify_subscriber(db, sub, alert_data)
        total_sent += sum(1 for log in logs if log.status == "sent")

    logger.info(f"Alert {alert.id}: notified {total_sent} channels across {len(matched_subs)} subscribers")
    ...
    try:
        ...
        async with async_session() as db:
            for alert_data in alerts:
                ...
                # Atomic dedup check via Redis
                if await is_new_alert(dedup_hash):
                    new_count += 1
                    try:
